...
# 
# This example demonstrates how to use Exabyte RESTful API to create simulation [Jobs](
# https://docs.exabyte.io/jobs/overview/) programmatically for multiple [Materials](
# https://docs.exabyte.io/materials/overview/) at once and extract the resulting [Properties](
# https://docs.exabyte.io/properties/overview/) forming a [Pandas](https://pandas.pydata.org/) dataframe.
# 
# This approach can work with any [Workflows](https://docs.exabyte.io/workflows/overview/). For the demonstration purpose we
# use the Density Functional Theory and extract Electronic Band Gap as the property of interest.
# 
# > <span style="color: orange">**IMPORTANT NOTE**</span>: In order to run this example in full, an active Exabyte.io
# account with access to VASP (Vienna ab-initio simulations package) is required. Alternatively, Readers may substitute the
# workflow ID below with another one (an equivalent one for Quantum ESPRESSO, for example) and adjust extraction of the
# results ("Extract results" section). RESTful API credentials shall be updated in [settings](../settings.py).
# 
# 
# ## Steps
# 
...
# Next, we retrieve the owner and project IDs as they are needed by the endpoints. Account's default material is used to
# extract the owner ID. One can extract the owner ID from any other account's [entities](
# https://docs.exabyte.io/entities-general/overview/).
...
# 
# - Band gaps are extracted from the second unit (vasp-bands with index 1) of the second job's subworkflow (SCF-BS-BG-DOS
# with index 1).
