...
from mat3ra.notebooks.utils.other.material import get_all_slabs_and_terms, freeze_center_bulk
...
with open(al2o3_poscar_filename, "r") as inp:
    content = inp.read()
    al2o3_cell_material = exabyte_materials_endpoint.import_from_file(
        name="Gamma_Alumina_Unit_Cell", content=content, owner_id=ORGANIZATION_ID
    )
...
job_config = {"ppn": 16, "queue": "OF", "nodes": 1, "time_limit": "00:20:00", "cluster": "cluster-007"}
compute = exabyte_jobs_endpoint.build_compute_config(**job_config)

# Create the Al2O3 job
al2o3_job = exabyte_jobs_endpoint.create_by_ids(
    [al2o3_cell_material], al2o3_cellopt_workflow["_id"], project_id, ORGANIZATION_ID, "al2o3_cellopt", compute
)
...
al2o3_materials_set = exabyte_materials_endpoint.create_set(
    {"name": "Webinar_Slabs_Al2O3", "owner": {"_id": ORGANIZATION_ID}}
)
