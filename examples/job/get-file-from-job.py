...
# 
# > <span style="color: orange">Not sure what's going on?</span>: This cell uses our API to copy the unit cell of silicon from Materials Project into your account. It then copies a workflow to get the total energy of a system using Quantum Espresso to your account. Finally, a job is created using the Quantum Espresso workflow for silico For more information, please refer to our [run-simulation-and-extract-properties](./run-simulations-and-extract-properties.ipynb) notebook, located in this directory.
...
for file in files:
    if file['name'] == 'pw_scf.out':
        outfile_metadata = file
JSON(outfile_metadata)
...
server_response = urllib.request.urlopen(outfile_metadata['signedUrl'])
outfile_bytes = server_response.read()
...
outfile = outfile_bytes.decode(encoding="UTF-8")
...
for file in files:
    if 'pw_scf.in' == file['name']:
        infile_metadata = file     
server_response = urllib.request.urlopen(infile_metadata['signedUrl'])
infile_bytes = server_response.read()
...
with open(infile_metadata['name'], 'wb') as outp:
    outp.write(infile_bytes)
...
with open(outfile_metadata['name'], 'w') as outp:
    outp.write(outfile)
