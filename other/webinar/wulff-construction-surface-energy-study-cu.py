...
def get_best_structure_by_formula(formula: str) -> pymatgen.ext.matproj.ComputedStructureEntry:
    ...
    cu_structs = materials_project_api.get_entries(
        formula, conventional_unit_cell=True, sort_by_e_above_hull=True, inc_structure="initial"
    )
...
with open(cu_poscar_filename, "r") as inp:
    content = inp.read()
    cu_cell_material = exabyte_materials_endpoint.import_from_file(
        name="Copper_Unit_Cell", content=content, owner_id=ORGANIZATION_ID
    )
...
kpoints_content = ["Automatic mesh", "0", "Gamma", "8 8 8", "0 0 0"]
...
cu_workflow = exabyte_bank_workflows_endpoint.copy(bank_workflow_id, account_id=ORGANIZATION_ID)
...
project_id = exabyte_projects_endpoint.list({"isDefault": True, "owner._id": ORGANIZATION_ID})[0]["_id"]

# Create the compute configuration for the jobs on Azure
exabyte_jobs_endpoint = JobEndpoints(*ENDPOINT_ARGS)
job_config = {"ppn": 4, "queue": "D", "nodes": 1, "time_limit": "00:10:00", "cluster": "cluster-001"}
compute = exabyte_jobs_endpoint.build_compute_config(**job_config)

# Create the Cu job
cu_job = exabyte_jobs_endpoint.create_by_ids(
    [cu_cell_material], cu_cellopt_workflow["_id"], project_id, ORGANIZATION_ID, "cu_cellopt", compute
)
...
cu_materials_set = exabyte_materials_endpoint.create_set(
    {"name": "Webinar_Cu_Wulff_Slabs", "owner": {"_id": ORGANIZATION_ID}}
)
...
for miller_index, term_dict in cu_slabs.items():
    for term, surface in term_dict.items():
        ...
        with open(filename, "r") as inp:
            content = inp.read()
        material_json = exabyte_materials_endpoint.import_from_file(
            name=material_name, content=content, owner_id=ORGANIZATION_ID
        )
...
kpoints_content = ["Automatic mesh", "0", "Gamma", "8 8 1", "0 0 0"]
...
workflow = exabyte_bank_workflows_endpoint.copy(bank_workflow_id, account_id=ORGANIZATION_ID)
...
for miller_index, term_dict in cu_slabs.items():
    for term, surface in term_dict.items():
        ...
        job_config = {"ppn": 16, "queue": "OF", "nodes": n_nodes, "time_limit": "12:00:00", "cluster": "cluster-007"}
        compute = exabyte_jobs_endpoint.build_compute_config(**job_config)
        ...
        cu_slab_job = exabyte_jobs_endpoint.create_by_ids(
            [material], workflow_id, project_id, ORGANIZATION_ID, job_prefix, compute
        )
...
for miller_index, term_dict in cu_slabs.items():
    for term, surface in term_dict.items():
        ...
        # Surface Energy
        surface_energy = get_surface_energy(e_slab=e_slab, e_bulk=e_bulk, n_slab=n_slab, n_bulk=n_bulk, a=area)
...
cluster_size = 147
wulff = ase.cluster.wulff_construction("Cu", surfaces=surfaces, energies=energies, size=cluster_size, structure="fcc")
